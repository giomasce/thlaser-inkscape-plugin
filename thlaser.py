        ...
        for arg in args.split(","):
            ...
            if field == 'power':
                try:
                    ...
                    raise ValueError("Invalid power '%s'" % (value))
            params[field] = value
...
class Gcode_tools(inkex.Effect):
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("-P", "--power",                    action="store", type="int",         dest="power", default="255",                        help="PWM power (0-255)")
        self.OptionParser.add_option("",   "--draw-order",               action="store", type="string",       dest="draw_order", default="inside_first",          help="Drawing order")
...
#            p = np[:]        
    ...
    def generate_gcode(self, curve, depth, altfeed=None, altpower=None):
        gcode = ''
        if (altfeed):
    ...
    def effect_curve(self, selected):
        ...
        for layer in layers:
            ...
            altpower = layerParams.get("power", None)
            ...
            def estimate_bb_area(curve):
                ...
                It just computes min and max of the handles, does not
                bother with computing the real bounding box of
                higher-order curves.
            ...
            if self.options.draw_order == 'inside_first':
                pathList.sort(key=estimate_bb_area)
            elif self.options.draw_order == 'outside_first':
                pathList.sort(key=estimate_bb_area, reverse=True)
            ...
            gcode += self.generate_gcode(curve, 0, altfeed=altfeed, altpower=altpower)
